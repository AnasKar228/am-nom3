...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    - константные колонки;
    - категориальные признаки с очень высокой кардинальностью;
    - подозрительные дубликаты в ID-колонках.
    """
    flags: Dict[str, Any] = {}

    # Базовые флаги из старой версии
    ...
    # 1) Константные колонки (все значения одинаковые среди ненулевых)
    constant_columns: List[str] = []
    for col in summary.columns:
        # считаем, что колонка "константная", если:
        # - есть хотя бы одно ненулевое значение
        # - число уникальных значений среди ненулевых <= 1
    ...
    # определяем категориальные как "не numeric"
    high_cardinality_categoricals: List[str] = []
    high_cardinality_details: Dict[str, float] = {}

    n_rows = summary.n_rows if summary.n_rows > 0 else 1  # защита от деления на ноль
    ...
    for col in summary.columns:
        if col.is_numeric:
            continue
        # доля уникальных значений среди всех строк
        unique_share = col.unique / n_rows
        if col.unique >= cardinality_min_unique and unique_share > cardinality_share_threshold:
            high_cardinality_categoricals.append(col.name)
            high_cardinality_details[col.name] = unique_share
    ...
    flags["high_cardinality_categoricals_unique_share"] = high_cardinality_details
    flags["n_high_cardinality_categoricals"] = len(high_cardinality_categoricals)

    # 3) Подозрительные дубликаты в ID-колонках
    # Ищем колонки, имя которых похоже на ID: "id", "*_id", "user_id" и т.п.
    id_like_columns = [
