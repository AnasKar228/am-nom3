...
def test_quality_flags_constant_and_id_duplicates():
    """
    Проверяем новые эвристики:
    - константные колонки;
    - дубликаты в ID-колонках (user_id).
    """
    df = pd.DataFrame(
        {
            # user_id повторяется -> должны детектиться дубликаты в ID
            "user_id": [1, 1, 2],
            # все значения одинаковые -> константная колонка
            "constant_col": [42, 42, 42],
            # обычная числовая колонка
            "value": [10, 20, 30],
        }
    )
